...
export const useProductItems = () => {
  ...
  const getDeliveryDate = () => {
    ...
    const date = new Date(
      currentDate.setDate(currentDate.getDate() + (Math.floor(Math.random() * (10 - 5 + 1)) + 5)),
    ).toLocaleString(undefined, options);
    return date;
  };

  const addToCartWishList = async (productId: string, type: "addToCart" | "addToWishList") => {
    if (!isLoginned) return router.push(`/login?referer=products&productId=${productId}&action=${type}`);
    try {
  ...
  const getwishListBody = (productId) => {
    ...
    wishList.forEach(({ item }) => {
      if (item?.id !== productId) wishListBody.push({ productId: item.id });
    });
    return wishListBody;
  };
