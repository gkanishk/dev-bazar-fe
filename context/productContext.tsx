...
export function ProductsProvider({ children }: Props) {
  ...
  const filterProducts = (
    ...
    if (idealFor.length > 0) tempProducts = tempProducts.filter(({ attributes }) => idealFor.includes(attributes.for));
    if (discounts.length > 0)
      tempProducts = tempProducts.filter(({ discount }) => discounts.includes(Number(discount)));
    if (sizes.length > 0)
      tempProducts = tempProducts.filter(
        ({ attributes }) => sizes.filter((element) => attributes.sizes.includes(element)).length > 0,
      );
    if (sortBy === "low") tempProducts = tempProducts.sort((a, b) => a.price - b.price);
    else if (sortBy === "high") tempProducts = tempProducts.sort((a, b) => b.price - a.price);
    if (search.length > 0) {
