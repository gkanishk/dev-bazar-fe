...
function LoginPage() {
  ...
  return (
    <div className="grid h-full place-items-center bg-red-50">
      <div className="p-4 bg-white rounded min-h-100 max-width-container">
        {isLogin ? (
          <LoginForm redirectPage={redirectPage} changePage={() => setIsLogin(!isLogin)} />
        ) : (
          <SignUpForm redirectPage={redirectPage} changePage={() => setIsLogin(!isLogin)} />
        )}
      </div>
    </div>
  );
